...
public partial class Interface
{
    ...
    private void DrawAutoGatherList(AutoGatherList list)
    {
        ...
        ImGui.SameLine();
        var onlyCurrentList = GatherBuddy.Config.AutoGatherConfig.OnlyUseCurrentAutoGatherList;
        if (ImGui.Checkbox("選択中リストだけ自動採取", ref onlyCurrentList))
        {
            GatherBuddy.Config.AutoGatherConfig.OnlyUseCurrentAutoGatherList = onlyCurrentList;
            GatherBuddy.Config.Save();
            if (onlyCurrentList)
                _plugin.AutoGatherListsManager.UseOnlyList(list);
        }
        if (ImGui.IsItemHovered())
            ImGui.SetTooltip("ONの間は、この画面で有効にしたリスト以外を自動でOFFにします。前に止めた未完了リストが小窓に残って優先されるのを防ぎます。");
